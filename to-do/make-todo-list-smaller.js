...
export function makeTodoSmaller(boxNumber) {
  ...
  createMassiveBox();
      ...
      for (let i = 0; i < boxesTodo.length; i++) {
            boxesTodo[i].querySelector('.makeList').addEventListener("mouseover", makeTodoBigger, {once: true});
            }
  }
...
export function makeTodoBigger(event){
  ...
  event.currentTarget.onmousedown = function() {
        ...
        while (0<todoList.length) {    // если добавят в todoList 
            todoList.pop();
            }
        ...
        let boxesTodo = document.querySelector('.category-boxes-todo').querySelectorAll('.category-box');
        for (let i = 0; i < boxesTodo.length; i++) {
            boxesTodo[i].querySelector('.makeList').removeEventListener("mouseover", makeTodoBigger, {once: true});
            }
         tmpTodoList = []; 
        }
